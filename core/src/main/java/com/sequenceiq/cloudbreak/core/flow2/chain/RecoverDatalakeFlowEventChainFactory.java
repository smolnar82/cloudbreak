...
public class RecoverDatalakeFlowEventChainFactory implements FlowEventChainFactory<ClusterRecoveryTriggerEvent> {
    ...
    public FlowTriggerEventQueue createFlowTriggerEventQueue(ClusterRecoveryTriggerEvent event) {
        ...
        flowEventChain.add(new TerminationEvent(FlowChainTriggers.RECOVERY_TERMINATION_TRIGGER_EVENT, event.getResourceId(), false));
        flowEventChain.add(new ClusterRecoveryTriggerEvent(RECOVERY_TEARDOWN_EVENT.event(), event.getResourceId(), event.accepted()));
