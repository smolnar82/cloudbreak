...
public class VirtualGroupTest extends AbstractMockTest {
    ...
    public void testCreateEnvironment(TestContext testContext) {
        ...
        testContext
                ...
                .then(this::validateEnvironmentCreatorUserRole)
    ...
    private EnvironmentTestDto validateResourceRights(TestContext testContext, EnvironmentTestDto testDto, EnvironmentClient environmentClient) {
        Map<String, List<RightV4>> resourceRightsToCheckForEnv = new HashMap<String, List<RightV4>>() {{
    ...
    private UmsRoleTestDto validateEnvironmentCreatorUserRole(TestContext testContext, UmsRoleTestDto testDto, UmsClient umsClient) {
        GetRightsResponse getRightsResponse = testDto.getResponse();
        Assertions.assertThat(
                getRightsResponse.getRoleAssignmentList().stream()
                        .anyMatch(roleAssignment -> roleAssignment.getRole().getCrn().equals(UmsRoleTestDto.ENVIRONMENT_CREATOR))).isTrue();
        return testDto;
    }
