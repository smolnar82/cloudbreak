...
public class YarnApplicationCreationService {
    ...
    public boolean checkApplicationAlreadyCreated(YarnClient yarnClient, String applicationName) throws MalformedURLException {
        ...
        boolean result = yarnClient.getApplicationDetail(applicationDetailRequest).getStatusCode() == YarnResourceConstants.HTTP_SUCCESS;
        LOGGER.debug("The yarn application " + applicationName + " already existing is " + result + ".");
        return result;
    }
    ...
    public void createApplication(YarnClient yarnClient, CreateApplicationRequest createApplicationRequest) throws MalformedURLException {
        LOGGER.debug("Creating the Yarn application " + createApplicationRequest.getName() + ".");
        ...
        if (Objects.nonNull(responseContext.getResponseError())) {
            LOGGER.error("Received a response error from the Yarn client when trying to create the application " + createApplicationRequest.getName() + ".");
